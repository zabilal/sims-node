...
describe('Auth routes', () => {
  ...
  describe('POST /v1/auth/forgot-password', () => {
    ...
    test('should return 204 and send reset password email to the user', async () => {
      ...
      await request(app).post('/v1/auth/forgot-password').send({ email: userOne.email }).expect(NO_CONTENT);
    ...
    test('should return 404 if email does not belong to any user', async () => {
      await request(app).post('/v1/auth/forgot-password').send({ email: userOne.email }).expect(NOT_FOUND);
    });
  });
...
describe('Auth middleware', () => {
  test('should call next with no errors if access token is valid', async () => {
    await insertUsers([userOne]);
    const req = createRequest({ headers: { Authorization: `Bearer ${userOneAccessToken}` } });
    const next = jest.fn();
  ...
  test('should call next with unauthorized error if access token is not found in header', async () => {
    ...
    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({ statusCode: UNAUTHORIZED, message: 'Please authenticate' })
    );
  });
